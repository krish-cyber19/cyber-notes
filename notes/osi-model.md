...
The third step is necessary and cant be combined with step two, as the server may still have data pending to be sent to the client after receiving 
the client's FIN.
...
MAC FLOODING - floods a switch's MAC address table with thousands of fake MAC addresses. Table fills up, switch enters "fail-open" mode — starts 
broadcasting all traffic out of every port like a hub. Attacker captures everything.
